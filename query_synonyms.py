#!/usr/bin/env python3
"""Search content for a user term and gene symbols, then export highlighted HTML.

The user term uses the Atlas Search ``phrase`` operator on ``body_text`` (``lucene.standard``,
default slop 0) so all entered words must appear in order. Gene symbols use a separate
``text`` clause on ``scientificGeneAnalyzer``. Atlas Search only allows ``synonyms`` in
queries against ``lucene.keyword`` fields, so canonical symbols from ``gene_synonyms.input``
are merged into the OR term list in Python (aliases in ``synonyms`` are not expanded).
"""
...
def _add_gene_terms(symbols: set[str], value: object) -> None:
    if value is None:
        return
    if isinstance(value, list):
        symbols.update(term for term in value if term)
    elif value:
        symbols.add(value)


def load_gene_search_terms(db: Database) -> list[str]:
    """Union homo_sapiens symbols with canonical gene_synonyms input symbols only."""
    symbols = {
        doc["_id"]
        for doc in db.homo_sapiens_gene_info.aggregate([{"$group": {"_id": "$Symbol"}}])
        if doc["_id"]
    }
    for doc in db.gene_synonyms.find({}, {"input": 1}):
        _add_gene_terms(symbols, doc.get("input"))
...
def build_search_pipeline(search_terms: list[str]) -> list[dict[str, Any]]:
    ...
    User phrase (required, no score impact)::

        {"phrase": {"path": "body_text", "query": "<USER_TERM>"}}

    Gene symbols (required, scored) on the ``scientificGeneAnalyzer`` multi field::

        {"text": {"path": {"value": "body_text", "multi": "scientificGeneAnalyzer"}, "query": [...]}}
    """
...
def find_gene_matches(body_text: str, gene_terms: set[str]) -> list[str]:
    """Unique gene/synonym tokens in body_text (case-sensitive scientific tokenization)."""
...
<body>
  ...
  <div class="meta">
    ...
    <div><strong>Gene terms:</strong> homo_sapiens symbols + gene_synonyms.input (canonical only)</div>
    <div><strong>Generated:</strong> {generated_at}</div>
    <div><strong>Gene search term load:</strong> {timings["terms"]:.3f}s</div>
    <div><strong>Content $search query:</strong> {timings["search"]:.3f}s</div>
  </div>
  <div class="legend">
...
def main() -> None:
    ...
    search_terms, terms_elapsed = measure_query_time(
        "Gene search term load",
        lambda: load_gene_search_terms(db),
    )
    ...
    html_content = render_results_html(
        ...
        timings={"terms": terms_elapsed, "search": search_elapsed},
    )
